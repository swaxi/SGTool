...
def get_processor_class():
    ...
    try:
        ...
        content = content.replace('from ..worms.wormer import Wormer', '# from ..worms.wormer import Wormer')
        content = content.replace('from ..worms.Utility import', '# from ..worms.Utility import')
        ...
        def fill_nan_simple(data):
            ...
            for i in range(data.shape[0]):
                ...
                if np.any(valid) and not np.all(valid):
                    ...
                    filled[i, ~valid] = np.interp(indices[~valid], indices[valid], row[valid])
            ...
            for j in range(data.shape[1]):
                ...
                if np.any(valid) and not np.all(valid):
                    ...
                    filled[~valid, j] = np.interp(indices[~valid], indices[valid], col[valid])
        ...
        namespace.update({
            ...
            'insert_text_before_extension': lambda path, text: os.path.splitext(path)[0] + text + os.path.splitext(path)[1],
...
class ArcGISProcessor:
    def process_raster_tool(self, input_raster, output_raster, processing_method, **kwargs):
        ...
        try:
            ...
            convert_to_degrees = kwargs.pop('convert_to_degrees', False)  # This one can be removed
            
            processor = ProcessorClass(dx, dy, init_buffer_size)
            
            # Check if method exists
            if not hasattr(processor, processing_method):
                available = [m for m in dir(processor) if not m.startswith('_') and callable(getattr(processor, m))]
                arcpy.AddError(f"Method '{processing_method}' not found. Available: {available[:5]}")
                return False
...
class RemoveRegionalTrend(object):
    ...
    def getParameterInfo(self):
        ...
        params[-1].description = "1st order removes linear gradients, 2nd order removes curved regional trends"
    ...
    def _process_gradient_removal(self, input_raster, output_raster, method_name):
        ...
        try:
            ...
            processor = ProcessorClass(dx, dy, 100)  # Buffer size not used for these methods
            ...
            if hasattr(processor, method_name):
                ...
                arcpy.AddError(f"Method {method_name} not found in GeophysicalProcessor")
                return False
...
class AutomaticGainControl(object):
    ...
    def execute(self, parameters, messages):
        ...
        if not success:
            raise arcpy.ExecuteError("Upward continuation failed")

class DownwardContinuation(object):
    def __init__(self):
        self.label = "Downward Continuation"
        self.description = "Enhance high-frequency signals (use with caution - amplifies noise)"
        self.canRunInBackground = False
...
class VerticalIntegration(object):
    def __init__(self):
        ...
        self.description = "Integrate field vertically to simulate deeper equivalent sources"
        self.canRunInBackground = False
